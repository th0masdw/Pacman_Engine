...
void Renderer::RenderTexture(const Texture2D& texture, const glm::vec2& pos) const
{
	SDL_Rect dst;
...
void Renderer::RenderTexture(const Texture2D& texture, const glm::vec2& pos, float angle, const glm::vec2& scales) const
{
	SDL_Rect dst;
	SDL_QueryTexture(texture.GetSDLTexture(), nullptr, nullptr, &dst.w, &dst.h);
